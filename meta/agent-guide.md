...
For workflow:
- **`scripts/update.sh`** — the weekly all-source check (scrape → process → build; `--verify` opts into URL verification of the new rows). `meta/automation-prompt.md` is the retired routine prompt, kept for history.
- **`meta/backlog-prompt.md`** — expanding coverage for a single source that already has entries.
- **`meta/automation-log.md`** — run log of the retired claude.ai routine (history). Cloud runs are recorded in their PRs and the Actions job summaries.
...
scripts/                       # all Python tooling
  ...
  curate.py                   # single-entry edits (exclude / reactivate / set-description / set-tags / set-type)
...
| Metric | Value |
|---|---|
| **Total published entries** | 3,912 (as of 2026-09-21, after the first cloud weekly update, PR #3: +8 entries #9018–9027). Earlier baseline 3,904 (as of 2026-09-14; 4,030 active in hub.db, of which 126 with broken URLs are held out of published outputs — every published row up to #9001 has been URL-verified as of 2026-08-31, verification is off by default since 2026-09-06; UChicago Consortium 31 -> 320, WestEd 14 -> 912, EdTrust 31 -> 322 and NWEA Research 70 -> 291 backfilled on 2026-08-30, with out-of-scope-type items recorded as excluded `type_filtered` rows: 364 WestEd, 66 EdTrust, 18 NWEA. Brookings: weekly scraping off, 23 selected entries published, its 2026-08-30 rows kept as excluded `source_on_hold` rows. Genre rule in `docs/purpose.md` § Scope. WWC rated only from 2026-08-30: 144 tier 1–3 intervention reports + 30 practice guides published, 475 Tier −1 reports held as `wwc_tier_minus1_no_evidence`; practice guides scanned weekly via `sources/wwc-practice-guides.json`. Evidence for ESSA rated only from 2026-08-31: 203 published (Strong / Moderate / Promising; the 4 rated programs past page 1 backfilled), 31 No Evidence / N/A rows held as `essa_no_evidence` via `evidence-for-essa.json` `exclude_when`) |
| **Structured metadata** | As of 2026-09-16: `published_date` on 3,305 published entries (392 marked `n/a`: Evidence for ESSA, Campbell, and non-publication types; 207 undated), `authors` on 2,679, `document_url` on 493 (exposed in `data.json`, `llms-full.txt` and the MCP worker), `grade_level` on 245 (WWC only, not exposed). Per-source notes in each `sources/*.md` under "Metadata backfill (2026-09-16)"; remainder on `roadmap.md` |
| **Weekly update** | Runs on GitHub Actions (`.github/workflows/weekly-update.yml`, manual trigger, no schedule yet): `scripts/update.sh`, then the `/weekly-update` skill reviews the new rows, then a separate job opens a PR from `auto/weekly`. 14 sources on the weekly list; Campbell is off it since 2026-09-21 (refuses GitHub-hosted runners) and runs locally. Details in `meta/operator-guide.md` |
...
scrape.py features: `url_filter` (filter API results by a listing page), `detail_fetch` (fetch individual pages for descriptions), path-based pagination, progress save/resume on interruption, early-stop (opt-in per config with `"early_stop": true` for listings known to be newest-first: stop after 3 consecutive or 5 total already-indexed URLs on a page; `--pages` is the hard cap, `--no-diff` disables). Backlog items are written into the staging JSON and recorded by `process_staged.py` as excluded `no_description_pending` rows. Each inserted row records `description_source` (`listing`, or the `detail_fetch` label `page-meta` / `page-abstract`; see `docs/schema.md`). `request_delay` has a 5 s floor; two consecutive 403/451 responses from the source's own host stop the run (likely IP/WAF block; switch egress before retrying). Structured metadata (`published_date`, `authors`, `grade_level`, `document_url`, each with a provenance column: `listing`, `page-meta`, `url`, `prose`, `llm`, `manual`) is filled on existing rows by `python scripts/process_staged.py {source} --backfill-metadata` after a `scrape.py {source} --no-diff` pass, or with `--from-db` from the rows' stored raw items (no requests); `scrape.py {source} --from-db` re-fetches a source's indexed pages for page-only fields; `discovery: "oai"` covers the OJS journals. Every 200 response is kept gzipped in the gitignored `data/raw/{source}/` sidecar so a later field is a local pass. Config options (`fetch_all`, `multiple`, `regex`, `jsonld`, `metadata_map`, `date_from_url`, `from_db`) are documented in `sources/README.md`. Backfill walks run at `request_delay` 12 s.
