...
- `exclude_when`: list of `{"field", "values", "reason"}` rules; an item whose field value (case-insensitive) is in `values` is staged as a `filtered_items` reject with `filter_reason` = `reason`, which must be an exclude reason `curate.py` accepts (WWC: `evidence_tier` `-1` → `wwc_tier_minus1_no_evidence`). Items without the field are kept.
