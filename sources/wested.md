...
Metadata backfill 2026-09-16: `detail_fetch` (no description selector, `fetch_all`) reads the author block (`.ctb-name` per author), the copyright year (`Copyright: YYYY`, the only date the page states — the listing's cover-image alt text carries a month but is free prose and was not used) and the "Get This Resource" S3 file link as `document_url`. Full walk at 12 s: 142 listing pages + 915 item pages (~1,060 requests over four launches; a local DNS outage stopped one). 845/915 rows dated (year, `page-meta`), 805 authored, 380 with a PDF link — the `document_url` selector was added mid-walk, so the ~400 pages fetched before it need one more pass; pages fetched after 01:57 are in `data/raw/wested/`. 70 pages carry no copyright line.
