...
| Consume the corpus as an LLM (index → per-source files) | `docs/llms.txt` |
...
| **LLM consumption** (llms.txt standard) | `docs/` (llms.txt index, llms-<source>.txt parts, llms-full-<source>.txt shards, llms-full.txt) |
...
- **`llms.txt`** — root index: one line per source linking to `llms-<source>.txt` (compact lists, split into parts under 100,000 characters) and `llms-full-<source>.txt` (with descriptions), plus the tag vocabulary. Entry point for agents that follow links.
- **`llms-full.txt`** — all entries with descriptions + auto-generated nav header in one self-contained file, for clients that can only load a single URL.
...
- **`how-to-use.md`** — guide to accessing the hub (web UI, llms.txt, MCP, Gem, data.json).
