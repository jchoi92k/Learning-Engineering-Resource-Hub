...
docs/llms-full.txt   ← canonical source. Hand-edited by agents or the routine.
        |
        | `cd docs && python build_tags.py`  (run automatically by the routine; also runnable manually)
        ↓
...
`build_tags.py` is the only build step. It validates UTF-8 encoding, regenerates the auto-generated header at the top of `llms-full.txt`, and emits the four derived files above. **Always run it after editing `llms-full.txt`**, and commit the result.
...
- **Knowledge file** — `docs/gem-knowledge.txt`. Regenerated by `build_tags.py` on every build.
