...
| **Structured metadata** | As of 2026-09-16: `published_date` on 3,305 published entries (392 marked `n/a`: Evidence for ESSA, Campbell, and non-publication types; 207 undated), `authors` on 2,679, `document_url` on 493 (stored, exposure pending), `grade_level` on 245 (WWC only, not exposed). Per-source notes in each `sources/*.md` under "Metadata backfill (2026-09-16)"; remainder on `roadmap.md` |
