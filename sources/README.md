...
- `exclude_when`: list of `{"field", "values", "reason"}` rules; an item whose field value (case-insensitive) is in `values` is staged as a `filtered_items` reject with `filter_reason` = `reason`, which must be an exclude reason `curate.py` accepts (WWC: `evidence_tier` `-1` → `wwc_tier_minus1_no_evidence`; Evidence for ESSA: `evidence_rating` `No Evidence`/`N/A` → `essa_no_evidence`). Items without the field are kept.
- `type_default`: label given to every item the listing leaves untyped (`wwc-practice-guides.json`: `Practice Guide`, which `process_staged.py` maps to `framework`).
...
- `url_transform`: `{"replace": ..., "with": ...}` applied to every item URL before the diff, for API and selector-based sources alike; the LPI configs use it to drop the `/index%2Ephp/` (Drupal front-controller) prefix that a listing link occasionally carries, so the stored URL is the canonical path.
...
- `detail_fetch` metadata options (2026-09-16): `selector` is optional, so a block with only `extra_fields` fetches pages for metadata and leaves the listing blurb alone (WestEd, UChicago); `fetch_all: true` fetches every item, not only those with short blurbs (a resumed run skips pages it already has); an `extra_fields` spec may add `"multiple": true` (one value per matching element: author lists), `"regex"` (group 1 is the value: `"Copyright:\\s*(\\d{4})"`), or `"jsonld": "datePublished"` (read the key from the page's schema.org block instead of a selector). Fields the page supplied are recorded in the item's `detail_fields` and label `page-meta`.
- `metadata_map`: `{"date": "date_page", "authors": "authors_page"}` — copy a value the scrape stored under another key onto the field the metadata registry reads; the mapped key overrides the field (NWEA, whose API post date is wrong on most rows while the page byline and date were stored by the detail fetch).
- `date_from_url`: `[{"host": "e4.northwestern.edu", "regex": "/(\\d{4})/(\\d{2})/(\\d{2})/"}, {"regex": "-(20\\d{2})/?$"}]` — infer a date from the URL when nothing else supplied one; the regex groups are year, month, day and the value is stamped `date_source: url`. Only for sources whose pages state no date (CREDO slugs, WordPress permalink paths).
- `from_db`: `{"source_names": [...], "host_allow": [...]}` — enables `python scripts/scrape.py <src> --from-db`: no discovery, the source's active hub.db rows are the items and the config's `detail_fetch` runs over every one (type filters skipped). For metadata passes over pages already indexed; `discovery: "db"` marks a config that exists only for this (AIMS, IES REL). `host_allow` limits a multi-host source to hosts whose pages we have read.
- `discovery: "oai"`: OAI-PMH `ListRecords` in `oai_dc` (OJS journals: JEDM, JLA) — `discovery_url` is the ListRecords URL, `resumptionToken` pages follow; each record yields the article URL, title, abstract, `dc:date`, creators, subjects, DOI and the galley link as `document_url`. One request per 100 records; JLA's robots.txt asks for 60 s between them.
- Every 200 response a run receives is also written, gzipped, to the gitignored `data/raw/<src>/` sidecar (`<url hash>.html.gz` / `.json.gz` plus `index.tsv`), so a field mapped later can be read from the stored page instead of fetching it again.
- `process_staged.py <src> --backfill-metadata [--from-db] [--overwrite]`: fill `published_date`, `authors`, `grade_level`, `document_url` and `source_subjects` on existing rows matched by URL from the staging file (or, with `--from-db`, from each row's stored `raw_item`), storing the staged item as `raw_item` when the row has none. Never inserts, never touches descriptions or tags.
- `skip_on_cloud_runner` (string, optional): the reason this source must not be scraped from a GitHub-hosted runner (a host that answers datacenter IPs with a challenge page). `scripts/update.sh` skips the source when it runs on GitHub Actions, lists it as `skipped (local run only)` in the run summary, and makes no request; everywhere else the source runs as usual. `scrape.py` itself ignores the key.
