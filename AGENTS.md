...
- **One row per URL.** `hub.db` enforces uniqueness on `lower(rtrim(url,'/'))` across published and excluded rows (index `idx_entries_url_norm`; `build_from_db.py --check` reports violations). Writers compare with `scrape.url_key()` before inserting; a duplicate is deleted or re-pointed with `curate.py set-url`, never inserted alongside.
