...
- `raw_item` — everything the scraper collected for the row, as staged (JSON): listing or API fields such as authors, date and the publisher's type label, evidence fields where a source provides them, and for detail-fetched pages the `page_meta` block (meta description, og:*, published time, citation / Dublin Core tags, schema.org datePublished, canonical URL). Kept so later passes (tagging, type review, description upgrades) never need to fetch the page again. Rows inserted before 2026-08-31 have it only where a later re-scrape or a `--backfill-metadata` pass (which stores the staged item when the row has none, since 2026-09-16) filled it. The raw HTTP responses themselves (page HTML, API and OAI records) are kept outside the database, gzipped under the gitignored `data/raw/<source>/` sidecar with an `index.tsv`, so a field mapped later is a local pass rather than a new crawl.
- `grade_level` / `grade_level_source` — the source's own grade band, stored where a source states one (What Works Clearinghouse only, as of 2026-09-16) and deliberately not published: only WWC and Evidence for ESSA expose one, so the field would be empty on 9 entries in 10.
