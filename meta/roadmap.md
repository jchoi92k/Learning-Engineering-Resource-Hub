...
- **`document_url` coverage.** The direct report link is exposed (`data.json`, `llms-full.txt`, MCP `search_resources` / `get_entry` / `fetch`, `get_stats.document_links`) on 493 rows (WestEd, JEDM, JLA, TNTP, AIMS). Coverage passes still to run: Mathematica (`mprpublicationurl` is in the API record; ~150 API pages, now kept in the raw sidecar), the ~400 WestEd pages fetched before the selector existed, UChicago / LPI / NWEA item pages. WWC PDFs sit behind JavaScript and Digital Promise bitstreams need a per-item call.
