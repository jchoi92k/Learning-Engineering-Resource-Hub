...
|---|---|
| `ok` | Nothing. |
| `skipped (local run only)` | Nothing to fix: the source's config opts out of cloud runners (`skip_on_cloud_runner`) and no request was made. Copy the summary's "Skipped on this cloud runner" line under "Needs a human" so the maintainer remembers the local run. |
...
The PR is public and read by people outside the project. Write it in a neutral register: say "run locally" or "a local run" for anything that cannot run on the cloud runner, and leave out details of any one maintainer's setup (their device, network, IP address, accounts).
