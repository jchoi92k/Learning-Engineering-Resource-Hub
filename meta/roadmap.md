...
- **Publication-date backfill.** `published_date` (with `date_source`) and `authors` exist in hub.db and are exposed in `data.json` and the MCP worker (`sort_by: "date"` / `"recently_added"`), but only Digital Promise rows carry values so far. Remaining: a per-source re-scrape with `process_staged.py --backfill-metadata`, in this order — LPI (selectors already mapped), Mathematica (dates come back in the same pass as its tag recovery), then WestEd, TNTP and WWC after their configs gain date/author selectors (WWC needs a detail fetch). Evidence for ESSA and Campbell publish no real date (their API date is the CMS post date) and are marked not available at build time; do not chase them. Prose-year extraction stays off unless stamped `date_source: prose`.
